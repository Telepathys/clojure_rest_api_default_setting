...
The `home-routes` handler in the `clojure-rest-api-default-setting.routes.home` namespace
...
The `home-page` function will in turn call the `clojure-rest-api-default-setting.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `clojure-rest-api-default-setting.handler` namespace:
...
Request middleware functions are located under the `clojure_rest_api_default_setting.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clojure-rest-api-default-setting.dev-middleware` namespace found in
